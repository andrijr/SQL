...
create  xml index secpk_tblXML_Value on #tblXML(xmlCol) 
			using xml index pk_tblXML FOR VALUE --nadajê siê kiedy uzywamy symboli wieloznacznych

create  xml index secpk_tblXML_Property on #tblXML(xmlCol) 
			using xml index pk_tblXML FOR PROPERTY -- nadajê siê kiedy pobieranych jest wiele wartoœci
